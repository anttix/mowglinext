...
auto_detect_position() {
  ...
  while [[ $attempt -lt 12 ]]; do
    fix_data=$(docker_cmd exec mowgli-ros2 bash -c "source /opt/mowgli_underlay.sh && source /ros2_ws/install/setup.bash && timeout 5 ros2 topic echo /gps/fix --once 2>/dev/null" 2>/dev/null || true)
  ...
  if docker_cmd inspect -f '{{.State.Status}}' mowgli-ros2 2>/dev/null | grep -q running; then
    local status_data
    status_data=$(docker_cmd exec mowgli-ros2 bash -c "source /opt/mowgli_underlay.sh && source /ros2_ws/install/setup.bash && timeout 5 ros2 topic echo /hardware_bridge/status --once 2>/dev/null" 2>/dev/null || true)
