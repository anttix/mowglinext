...
void FusionGraphNode::OnGnss(sensor_msgs::msg::NavSatFix::ConstSharedPtr msg)
{
  ...
  // Reject fixes without trustworthy accuracy before they can seed the datum or
  // mutate the wrong-fix gate's accepted/pending references.
  const double var_x = msg->position_covariance[0];
  const double var_y = msg->position_covariance[4];
  const auto usable_sigma =
      UsableGnssSigma(var_x,
                      var_y,
                      msg->position_covariance_type !=
                          sensor_msgs::msg::NavSatFix::COVARIANCE_TYPE_UNKNOWN,
                      wheel_vx_,
                      gps_sigma_speed_coeff_,
  ...
  if (!usable_sigma)
  {
    ...
    RCLCPP_WARN_THROTTLE(get_logger(),
                         *get_clock(),
                         5000,
                         "fusion_graph: GPS fix with unusable covariance/accuracy "
                         "(cov_type=%u, var_x=%.4g, var_y=%.4g) — rejected before "
                         "wrong-fix reference update",
                         msg->position_covariance_type,
                         var_x,
                         var_y);
    last_gps_sigma_ = -1.0;
  ...
  const double sigma = *usable_sigma;
