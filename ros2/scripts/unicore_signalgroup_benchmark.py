...
def build_remote_benchmark_script(args: argparse.Namespace, remote_results_dir: str) -> str:
  ...
  return textwrap.dedent(
      ...
      capture_state() {{
        local tag="$1"
        docker exec mowgli-ros2 bash -lc 'source /opt/mowgli_underlay.sh && source /ros2_ws/install/setup.bash && timeout '"$GPS_STATUS_TIMEOUT_SECONDS"' ros2 topic echo /gps/status --once' \
          >"$RESULTS_DIR/${{tag}}_gps_status.yaml" 2>"$RESULTS_DIR/${{tag}}_gps_status.stderr" || true
        docker exec mowgli-ros2 bash -lc 'source /opt/mowgli_underlay.sh && source /ros2_ws/install/setup.bash && timeout '"$DIAGNOSTICS_SECONDS"' ros2 topic echo /diagnostics' \
...
PY
      ...
      docker start mowgli-gps >"$RESULTS_DIR/${{tag}}_docker_start.log" 2>&1
      sleep 30
      docker exec mowgli-ros2 bash -lc 'source /opt/mowgli_underlay.sh && source /ros2_ws/install/setup.bash && timeout 12 ros2 topic echo /gps/status --once' \
        >"$RESULTS_DIR/${{tag}}_gps_status.yaml" 2>"$RESULTS_DIR/${{tag}}_gps_status.stderr" || true
      """)
